...
class CustomProviderConfig(BaseProviderConfig):
    """Custom provider configuration for non-preset providers.

    Supports format conversion between Anthropic and OpenAI protocols,
    custom base URLs (per-format), and configurable authentication styles.
    """
    type: Literal[ModelProviderTypeEnum.CUSTOM]
    customBaseUrl: Optional[str] = PydanticField(
        default=None,
        json_schema_extra={"field_required": True},
        description="Primary base URL (used for OpenAI format or as default)",
    )
    customBaseUrlAnthropic: Optional[str] = PydanticField(
        default=None,
        description="Anthropic-specific base URL. When set, used for Anthropic format requests. "
                    "Falls back to customBaseUrl if not set.",
    )
    supported_formats: List[str] = PydanticField(
        default_factory=lambda: ["openai"],
        description="Supported response formats: ['openai'], ['anthropic'], or ['openai', 'anthropic']",
    )
    auth_style: Optional[str] = PydanticField(
        default="bearer",
        description="Auth style for OpenAI format: bearer, x-api-key, or auto",
    )
    auth_style_anthropic: Optional[str] = PydanticField(
        default=None,
        description="Auth style for Anthropic format. Falls back to auth_style if not set.",
    )
    strip_fields: Optional[bool] = PydanticField(
        default=False,
        description="Strip non-standard fields when proxying to upstream",
    )
    request_format: Optional[str] = PydanticField(
        default=None,
        description="Expected request format for the upstream: 'openai' or 'anthropic'",
    )
    response_format: Optional[str] = PydanticField(
        default=None,
        description="Expected response format from the upstream: 'openai' or 'anthropic'",
    )

    def get_base_url(self, fmt: str = None) -> Optional[str]:
        """Get base URL for the specified format.

        Args:
            fmt: Optional format ("openai" or "anthropic").
                 If None, returns the primary URL.

        Returns:
            The base URL for the specified format, or None.
        """
        if fmt == "anthropic" and self.customBaseUrlAnthropic:
            return self.customBaseUrlAnthropic.rstrip("/")
        if self.customBaseUrl:
            return self.customBaseUrl.rstrip("/")
        return None
    ...
    def get_chat_url(self) -> Tuple[Optional[str], Optional[str]]:
        ...
        if not base_url:
            return None, None
        # Determine chat URI based on supported formats
        if "anthropic" in self.supported_formats and "openai" not in self.supported_formats:
...
class ModelProviderBase(SQLModel):
    ...
    supported_formats: Optional[List[str]] = Field(
        ...
        description="Supported response formats: ['openai'], ['anthropic'], or ['openai', 'anthropic']. "
                    "When set, enables format conversion routing.",
    )
    auth_style: Optional[str] = Field(
        default=None,
        nullable=True,
        description="Auth style for upstream: bearer, x-api-key, or auto",
    )
    strip_fields: Optional[bool] = Field(
        default=None,
        nullable=True,
        description="Strip non-standard fields (thinking, metadata, etc.) when proxying",
    )
