...
def _parse_model_list(content) -> List[Dict[str, Any]]:
    """Parse model list from upstream response, compatible with various formats.

    Supports:
    - OpenAI: {"data": [...]}
    - Various: {"models": [...]}, {"list": [...]}, {"items": [...]}
    - Nested: {"data": {"list": [...]}}
    - Plain list: [...]
    - String list: ["model-a", "model-b"]
    """
    source = None
    if isinstance(content, dict):
        for key in ("data", "models", "list", "items"):
            if key in content and isinstance(content[key], list):
                source = content[key]
                break
        if source is None and "data" in content and isinstance(content["data"], dict):
            source = content["data"].get("list")
    ...
    for m in source:
        if isinstance(m, str):
            ...
            mid = m.get("id") or m.get("name") or m.get("model") or m.get("model_id")
            if mid:
...
async def get_models_from_provider(
    ...
    url = build_openai_url(base_url, model_uri)
    data = []
    async with httpx.AsyncClient(
        ...
        try:
            ...
            data = _parse_model_list(content)
