...
from gpustack.converter.provider_proxy import proxy_to_provider
...
async def anthropic_messages(
    ...
    Handles requests in Anthropic format and routes them to appropriate
    backends, with format conversion when needed.
    """
    body = await _parse_anthropic_body(request)
    model_name = body.get("model")
    stream = body.get("stream", False)

    if not model_name:
        return _anthropic_error_response(400, "Missing 'model' field")
    ...
    if not await UserService(session).model_allowed_for_user(
        ...
        return _anthropic_error_response(404, "Model not found")

    # Try to find a provider-based route target first
    provider, provider_model_name, route = await _find_provider_for_model(
        session, model_name
    )

    if provider:
        # Route to external provider with format conversion
        return await proxy_to_provider(
            request=request,
            provider=provider,
            model_name=provider_model_name or model_name,
            request_format="anthropic",
            body=body,
            stream=stream,
        )

    # Try local model instances
    models: List[Model] = await ModelRouteService(
        session
    ).get_model_ids_by_model_route_name(model_name)
    if not models:
        return _anthropic_error_response(
            404, "Model not found or no running instances available"
        )
    ...
    if not worker:
        return _anthropic_error_response(
            500, f"Worker with ID {instance.worker_id} not found"
        )

    # For local instances, proxy directly
    url = f"http://{instance.worker_ip}:{worker.port}/proxy/v1/messages"
    token = worker.token
    extra_headers = {
        "X-Target-Port": str(instance.port),
        "Authorization": f"Bearer {token}",
    ...
    if model.backend == BackendEnum.ASCEND_MINDIE:
        url = f"http://{instance.worker_ip}:{instance.port}/v1/messages"
        extra_headers = {}

    logger.debug(
        f"anthropic proxying to {url}, instance port: {instance.port}"
    )

    try:
        if stream:
            return await _anthropic_stream_to_local(
                request, url, body, extra_headers
            )
        else:
            return await _anthropic_non_stream_to_local(
                request, url, body, extra_headers
            )
    except asyncio.TimeoutError:
        return _anthropic_error_response(
            529, f"Request to {url} timed out"
        )
    except Exception as e:
        return _anthropic_error_response(
...
async def _parse_anthropic_body(request: Request) -> dict:
    """Parse Anthropic Messages API request body."""
    try:
        return await request.json()
    except Exception as e:
        raise BadRequestException(
            message=f"Could not parse JSON body: {e}",
            is_openai_exception=True,
        )
...
async def _find_provider_for_model(
    ...
    """Find a provider-based route target for the given model name.

    Returns (provider, provider_model_name, route) or (None, None, None).
    """
    routes = await ModelRoute.all_by_field(
        session=session,
        field="name",
        value=model_name,
    )
    for route in routes:
        ...
        for target in targets:
            if target.deleted_at is not None or target.provider_id is None:
                continue
            provider = await ModelProvider.one_by_id(
                session=session, id=target.provider_id
            )
            if provider and provider.deleted_at is None:
                return provider, target.provider_model_name, route

    return None, None, None


async def _anthropic_stream_to_local(
    request: Request,
    url: str,
    body: dict,
    extra_headers: dict,
) -> Response:
    """Stream Anthropic-format response from local instance."""
    timeout = aiohttp.ClientTimeout(total=envs.PROXY_TIMEOUT)
    headers = filter_headers(request.headers)
    headers.update(extra_headers)
    ...
    async def stream_generator():
        ...
        async with http_client.post(
            ...
            if resp.status >= 400:
                ...
                yield error_body, resp.headers, resp.status
                return
            async for data in resp.content.iter_chunked(4096):
                yield data, resp.headers, resp.status
...
async def _anthropic_non_stream_to_local(
    request: Request,
    url: str,
    body: dict,
    extra_headers: dict,
) -> Response:
    """Handle non-streaming Anthropic request to local instance."""
    timeout = aiohttp.ClientTimeout(total=envs.PROXY_TIMEOUT)
    headers = filter_headers(request.headers)
    headers.update(extra_headers)
